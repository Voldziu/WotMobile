...
struct TanksView: View {
    ...
    @State private var visibleRange: Range<Int> = 0..<20 // Initial range of visible tanks
    private let batchSize = 20 // Number of tanks to load at a time
    ...
    var visibleTanks: [Tank] {
        // Safely return the current visible range of tanks
        let maxIndex = min(visibleRange.upperBound, filteredTanks.count)
        return Array(filteredTanks[visibleRange.lowerBound..<maxIndex])
    }
                    ...
                    ScrollView([.vertical, .horizontal], showsIndicators: true) {
                        LazyVStack(alignment: .leading, spacing: 0) {
                            ...
                            VStack(spacing:0){
                                ...
                                // List of visible tanks
                                LazyVStack(spacing:0) {
                                    ForEach(visibleTanks) { tank in
    ...
    private func loadMoreTanks() {
        let newUpperBound = min(visibleRange.upperBound + batchSize, filteredTanks.count)
